...
isco88_matches <- new_df %>% 
                  ...
                  summarise(isco88_matches = str_c(toString(isco88), collapse = ", ")) %>% ungroup()
...
full_df <- full_df %>% dplyr::select(term, osg, E, P, A, def_score, avg_siops, avg_sei, isco88_matches, isco08_matches)
...
kable(full_df, caption = "Occupational Identities",
      ...
      format = "pandoc", out = here("output/check_all_occ_id_table.html"))
```
...
sims_df <- sims_df %>% unnest_wider(opt_behavior) %>% separate(event, into = c("actor", "object"), sep = " ")
